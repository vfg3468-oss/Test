...
local Config = {
    ...
    StickyTele = false,
    TeleDistance = 3, 
    TeleSpeed = 20000, -- Mặc định tương đương 0.4 lerp
    CharAim = false,
    AutoSelect = false,
    AntiFling = false,
    WalkSpeed = 16,
...
CreateSlider("Tốc độ bay Tele Kill", 1, 50000, 20000, function(val) Config.TeleSpeed = val end) -- Thanh kéo mới
    ...
    if Config.Target and Config.Target.Character and not hasEscapedForLowHP then
        ...
        if tHrp then
            ...
            elseif Config.StickyTele then
                ...
                -- Tính toán tốc độ nội suy (Lerp Alpha) từ slider 1 - 50.000
                local lerpAlpha = math.clamp(Config.TeleSpeed / 50000, 0.001, 1)
                hrp.CFrame = hrp.CFrame:Lerp(targetBehindCFrame, lerpAlpha)
